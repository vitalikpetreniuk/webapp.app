...
class RevenueController extends Controller
{
    ...
    {
        return DB::select("SELECT TO_CHAR(date, 'Month') AS \"month\", EXTRACT(year from date) AS \"year\", MIN(date) as date, SUM(amount) as sum FROM revenues WHERE date BETWEEN ? AND ? GROUP BY 1, 2 ORDER BY date DESC", [$from, $to]);
    }
